...
function readStateFromFile(api, id) {
  var filename = _path["default"].join(api.user.storagePath(), "".concat(id, ".json"));
...
function getOn(api, id) {
  if (!id) throw 'No id passed to function';
  var state = readStateFromFile(api, id);
  return state.on;
}
/**
...
function setOn(api, id, value) {
  if (!id) throw 'No id passed to function';
  var state = readStateFromFile(api, id); // Update value
...
function getRollingCode(api, id) {
  if (!id) throw 'No id passed to function';
  var state = readStateFromFile(api, id);
...
function advanceRollingCode(api, id) {
  if (!id) throw 'No id passed to function';
  var state = readStateFromFile(api, id); // Advance code
