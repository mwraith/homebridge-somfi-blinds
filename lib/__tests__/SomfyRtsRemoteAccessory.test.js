...
jest.mock('pigpio'); // Need to mock timers for the admin buttons that switch off after X ms
...
describe("Testing Main Class", function () {
  ...
  test("check that admin buttons only advance code by 1", function () {
    // Config for the accessory
    var config = {
      "id": 99104,
      "name": "test",
    ...
    var rts = new _SomfyRtsRemoteAccessory["default"](log, config, api);
    var services = rts.getServices(); // Trigger admin buttons

    services[1].set(true); // Fast-forward until all timers have been executed

    jest.runAllTimers(); // Expect code to be 2
