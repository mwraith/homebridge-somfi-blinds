...
describe("Testing Window Covering Accessory", function () {
  ...
  test("Test current position works", function () {
    ...
    jest.advanceTimersByTime(config.blindOpenDelay * 0.3); // Expect current position to be 30% open

    CurrentPosition.get().then(function (data) {
      expect(data).toEqual(30);
    }); // Wait a while

    jest.advanceTimersByTime(config.blindOpenDelay * 0.3); // Expect current position to be 60% open
    ...
    jest.advanceTimersByTime(config.blindOpenDelay * 0.3); // Expect current position to be 30% closed

    CurrentPosition.get().then(function (data) {
      expect(data).toEqual(70);
    }); // Wait a while longer

    jest.advanceTimersByTime(config.blindOpenDelay * 0.3); // Expect current position to be 60% closed
