...
var WindowCoveringAccessory = /*#__PURE__*/function () {
  ...
  function WindowCoveringAccessory(name, log, config, api) {
    ...
    this.delay = this.config.blindOpenDelay || 5000; // Register a switch service
  ...
  (0, _createClass2["default"])(WindowCoveringAccessory, [{
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                percentComplete = (Date.now() - this.lastChange.timestamp) / this.delay; // Round to integer

                percentComplete = Math.round(percentComplete); // Ensure it hasn't gone over 100

                percentComplete = percentComplete > 100 ? 100 : percentComplete;
                this.log.debug("Getting current position for ".concat(this.config.id, " - ").concat(percentComplete, " from mocked value")); // The percentage complete can just be returned if the blind 
                ...
                return _context.abrupt("return", percentComplete);

              case 9:
                return _context.abrupt("return", 100 - percentComplete);
              ...
              case 12:
                this.log.debug("Getting current position from file"); // Otherwise the current position is the same as the target position
    ...
    key: "isBlindCurrentlyMoving",
    value: function isBlindCurrentlyMoving() {
          ...
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                ...
                internalState = value > 0 ? false : true;
                /*
                ...
                */
                ...
                button = this.config.invertToggle ^ internalState ? 'Down' : 'Up'; // Send button press to RpiGpioRts

                (0, _SendCommand.sendCommand)(this.config, button); // Persist the change to the file

                BlindState.setOn(this.config.id, internalState); // Track the request, which is used by getPositionState, getCurrentPosition
        ...
        return _regenerator["default"].wrap(function _callee3$(_context3) {
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                PositionState = this.api.hap.Characteristic.PositionState; // If the blind is currently moving then display the direction. Otherwise
                ...
                return _context3.abrupt("return", this.lastChange.targetValue > 0 ? PositionState.INCREASING : PositionState.DECREASING);
